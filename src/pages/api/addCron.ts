...
export const POST: APIRoute = async ({ request }) => {
    ...
    console.log(validSession.valid, url, name)
    if (validSession.valid && url && name && interval && daily_time && project_id) {
        ...
        if (project.rows.length > 0) {
            await connectdb().execute({
                ...
                args: [parseInt(project_id!.toString()), url.toString(), name!.toString(), interval!.toString(), daily_time!.toString(), request_type!.toString()],
            })
